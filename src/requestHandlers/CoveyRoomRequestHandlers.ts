...
export async function roomJoinHandler(requestData: RoomJoinRequest): Promise<RoomJoinResponse> {
  const coveyRoomStore = CoveyRoomsStore.getInstance();
  const coveyRoomController = coveyRoomStore.getControllerForRoom(requestData.coveyRoomID);
...
export function roomSubscriptionHandler(socket: Socket): void {
  ...
  const coveyRoomStore = CoveyRoomsStore.getInstance();
  const controller1 = coveyRoomStore.getControllerForRoom(coveyRoomID);

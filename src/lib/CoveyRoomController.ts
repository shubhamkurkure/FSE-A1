...
export default class CoveyRoomController {
  ...
  /** ID for a particular Covey Room * */
  private readonly _coveyRoomID: string;

  public constructor(_coveyRoomID: string) {
    this._coveyRoomID = _coveyRoomID;
  }
  ...
  async addPlayer(newPlayer: Player): Promise<PlayerSession> {
    ...
    theSession.videoToken = await this.videoClient.getTokenForRoom(this._coveyRoomID, newPlayer.id);
